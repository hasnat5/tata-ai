...
export async function POST(request: NextRequest) {
  try {
    ...
    const { message, attachments, conversationHistory, projectState } = body;

    // Build user content parts
    const userParts = await buildUserParts(message, attachments);

    // Drop any history entries with missing or empty parts. Vertex rejects the
    // whole request if any contents[i].parts is empty.
    const sanitizedHistory: GeminiContent[] = (conversationHistory ?? []).filter(
      (c) => Array.isArray(c.parts) && c.parts.length > 0,
    );

    // Only append a new user turn when we actually have parts to send. On
    // agent-loop iteration 2+, the client posts an empty message because the
    // tool functionResponse parts were already appended to history.
    const updatedHistory: GeminiContent[] =
      userParts.length > 0
        ? [...sanitizedHistory, { role: "user", parts: userParts }]
        : [...sanitizedHistory];

    if (updatedHistory.length === 0) {
      return NextResponse.json(
        {
          error: "Empty request: no message, attachments, or prior history",
        },
        { status: 400 },
      );
    }
