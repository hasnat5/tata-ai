...
export async function POST(request: NextRequest) {
  try {
    ...
    if (isEditing && existingThumbnailUrl) {
      try {
        const existingData = await fetchImageAsBase64(existingThumbnailUrl);
        contentParts.push({
          inlineData: {
...
- MUST maintain the EXACT ${aspectRatio} aspect ratio (${
        aspectRatio === "16:9" ? "landscape/horizontal" : "portrait/vertical"
      })
...
Generate the thumbnail image.`;
    ...
    for (const part of parts) {
      ...
      if (
        ...
        const filename = `thumbnail-scene-${sceneIndex + 1}-${Date.now()}.png`;

        try {
          thumbnailUrl = await uploadBase64Image(base64DataUrl, filename);
          console.log("Thumbnail uploaded to UploadThing:", thumbnailUrl);
        } catch (uploadError) {
